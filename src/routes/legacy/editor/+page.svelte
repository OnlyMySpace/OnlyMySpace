<script lang="ts">
	...
	$: isTimeZoneValid =
		profile.widget != null &&
		profile.widget.type == 'Time' &&
		profile.widget.widgetData != null &&
		(profile.widget.widgetData as TimeWidgetData).timezone != undefined &&
		validateTimezone({ target: { value: (profile.widget.widgetData as TimeWidgetData).timezone } });
	...
	>
		<div class="bg-error w-full h-12 text-center" class:hidden={hideWarning}>
			<p class="text-3xl font-bold">
				This page is deceprated. Please use the new editor instead <span
					on:click={() => (hideWarning = true)}
					class:hidden={hideWarning}
					tabindex={0}
